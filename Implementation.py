...
class HuffmanCoding:
    ...
    def __make_frequenqy_dict(self, text):
        freq_dict = {}
        for char in text:
    ...
    def __getBytesArrray(self, padded_encoded_text):
        array = []
    ...
    def compress(self):
        ...
        with open(self.path, "r+") as file, open(output_path, "wb") as output:
            ...
            freq_dict = self.__make_frequenqy_dict(text)
            # print(freq_dict)
            # Construct the heap from frequency dictonary
            self.__buildHeap(freq_dict)
            # Construct the binary Tree from the heap
            self.__buildTree()
            # Construct the codes from constructed text
            self.__buildCodes()
            # Creating the encoding text using the codes
            encoded_text = self.__getEncodedText(text)
            # print(encoded_text)
            # Pad this encoded_text
            padded_encoded_text = self.__getPaddedEncodedText(encoded_text)
            # return this binary file as output
            bytes_Array = self.__getBytesArrray(padded_encoded_text)
